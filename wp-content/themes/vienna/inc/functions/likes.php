...
if ( ( !$like_count ) || ( $like_count && $like_count == "0" ) ) { // no votes, set up empty variable
	...
	if ( AlreadyLiked( $post_id ) ) { // already liked, set up unlike addon
		$output .= '<span class="unliker"><i class="dashicons dashicons-no-alt"></i></span><span class="like prevliked"><i class="dashicons dashicons-heart"></i></span>';
		$output .= ' <span class="count alreadyliked">'.$likes.'</span></a></span>&nbsp; ';
	} else { // normal like button
		$output .= '<span class="unliker"></span><span class="like"><i class="dashicons dashicons-heart"></i></span>';
...
function show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php __( 'You Like:', 'zatolab' ); ?></label></th>
			<td>
