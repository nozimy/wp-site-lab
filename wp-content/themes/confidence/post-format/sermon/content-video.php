	...
	<?php $date = rwmb_meta('ninetheme_confidence_m_b_sermon_date', 'type=color'); ?>
	<?php $manager = rwmb_meta('ninetheme_confidence_m_b_sermon_manager', 'type=color'); ?>
	<?php $location = rwmb_meta('ninetheme_confidence_m_b_sermon_location', 'type=color'); ?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<div class="hentry-box">
			...
			<div class="post-thumb blog-bg-0ff <?php if ( $date =='' && $manager =='' && $location =='' ) : ?>m-b-30<?php endif; ?>">
			...
			<div class="post-thumb <?php if ( $date =='' && $manager =='' && $location =='' ) : ?>m-b-30<?php endif; ?>"><?php echo do_shortcode($ninetheme_confidence_m_b_wp_video); ?></div>

			<?php endif; ?>

			<?php if ( $date !='' || $manager !='' || $location !='' ) : ?>
			<div class="event-container">
				<div class="inner">
					<div class="inner items col-sm-3">
						<?php if(get_post_meta( get_the_ID(), 'ninetheme_confidence_m_b_sermon_date', true )){ 
						echo '<span class="lead-off-off uppercase strong">' . esc_html__('sermon date', 'confidence') . '</span> '; 
						echo '<span class="lead-off ">'.get_post_meta( get_the_ID(), 'ninetheme_confidence_m_b_sermon_date', true ).'</span> '; 
						} ?>
					</div>
					<div class="inner items col-sm-3">
						<?php if(get_post_meta( get_the_ID(), 'ninetheme_confidence_m_b_sermon_manager', true )){ 
						echo '<span class="lead-off uppercase strong">' . esc_html__('sermon manager', 'confidence') . '</span> '; 
						echo '<span class="lead-off ">'.get_post_meta( get_the_ID(), 'ninetheme_confidence_m_b_sermon_manager', true ).'</span> '; 
						} ?>
					</div>
					<div class="inner items col-sm-4">
						<?php if(get_post_meta( get_the_ID(), 'ninetheme_confidence_m_b_sermon_location', true )){ 
						echo '<span class="lead-off uppercase strong">' . esc_html__('Sermon Location', 'confidence') . '</span> '; 
						echo '<span class="lead-off ">'.get_post_meta( get_the_ID(), 'ninetheme_confidence_m_b_sermon_location', true ).'</span> '; 
						} ?>
					</div>
				</div>
			</div>
