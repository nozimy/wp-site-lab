...
if ( ! function_exists( 'ninetheme_confidence_breadcrubms' ) ) {
    function ninetheme_confidence_breadcrubms() {
        global $wp_query, $post, $paged;

        $space      = ' ';
        $on_front   = get_option( 'show_on_front' );
        $blog_page  = get_option( 'page_for_posts' );
        $separator  = $space . '<span class="bredcrumb-separator">' . apply_filters( 'ninetheme_confidence_breadcrumb_separator', '<i class="fa icon-caret-right"></i>' ) . '</span>' . $space;
        $link       = apply_filters( 'ninetheme_confidence_breadcrumb_link', '<a href="%1$s" title="%2$s" rel="bookmark" class="breadcrumb-item">%2$s</a>' );
        $current    = apply_filters( 'ninetheme_confidence_breadcrumb_current', '<span class="breadcrumb-current">%s</span>' );
        ...
        if ( $on_front == "page" && is_home() ) {
            $blog_title = isset( $blog_page ) ? get_the_title( $blog_page ) : esc_html__( 'Our Blog', 'confidence' );
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'confidence' ) ) . $separator . sprintf( $current, $blog_title );
        } else {
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'confidence' ) );
        }
        ...
        if ( is_singular() ) {
            ...
            if ( is_post_type_archive() ) {
                ...
                $out .= $separator . esc_html__( 'Category: ', 'confidence' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_tag() ) {
                $out .= $separator . esc_html__( 'Tag: ', 'confidence' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_date() ) {
                $out .= $separator;
                if ( is_day() ) {
                    global $wp_locale;
                ...
                $out .= $separator . sprintf( $current, esc_html__( 'Error 404', 'confidence' ) );
            } else if ( is_search() ) {
                $out .= $separator . sprintf( $current, esc_html__( 'Search', 'confidence' ) );
            }

        }

        $out .= '</p>';
        echo apply_filters( 'ninetheme_confidence_breadcrumbs_out', $out );
    }
}
// Hooks into the Digital Store theme
add_action( 'ninetheme_confidence_before_template_header', 'ninetheme_confidence_breadcrubms' );